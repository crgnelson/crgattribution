...
# Updated table_id to reflect the 60-day window
table_id = "ft-customer-analytics.crg_nniu.conversion_visit_static_60"

# Define the date range for processing
start_date = pd.to_datetime("2024-06-01")  # Start of the range
end_date = pd.to_datetime("2024-12-07") 

################################################# Output DataFrames  #########################################

# Updated variable names from _90 to _60
...
attribution_df_all = attribution_df_all_subs_60
normalized_removal_effects_all = normalized_removal_effects_all_subs_60
markov_transition_matrix_all = markov_transition_matrix_all_subs_60
user_df_all = user_df_all_subs_60
...
# Extract user_guid from journey_id (already done previously)
# Ensure 'user_guid' is present in user_df_all
if 'user_guid' not in user_df_all.columns:
...
def calculate_removal_effect(row):
    ...
    if pd.isna(attr) or pd.isna(channels) or pd.isna(ltv):
        return np.nan
...
user_df_trial_avg['conversion_window'] = 60
user_df_trial_avg['conversion_type'] = 'Subscription'
