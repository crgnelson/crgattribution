...
# ltv_table_id = "ft-customer-analytics.crg_nniu_attribution.stg_ltv_last_15_days"
...
def calculate_removal_effect_ltv(row):
    """From original code: multiplies each channel's attribution share by LTV."""
...
def process_user_df(user_df):
    """Explode removal_effect_ltv into channel-level ltv and avg per channel."""
    user_df = user_df.copy()
    user_df["removal_effect_ltv"] = user_df.apply(calculate_removal_effect_ltv, axis=1)
    ...
    avg = (
        df_exploded
        .groupby(["channel", "run_date", "conversion_window", "conversion_type"], as_index=False)
        .agg(average_ltv=("ltv", "mean"))
    )
    return avg
...
def run_pipeline(conversion_type, lookback_window):
    ...
    for current_date in pd.date_range(start_date, end_date):
        ...
        try:
            ...
            # merge product_arrangement_id
            df["conversion_visit_timestamp_date"] = pd.to_datetime(df["conversion_visit_timestamp"]).dt.date
            prod_df = (
                df[["user_guid", "conversion_visit_timestamp_date", "product_arrangement_id"]]
                .drop_duplicates()
            )
            user_df = user_df.merge(
                prod_df,
            ...
            # attribution_df_all
            attr = attributions.group_by_channels_models
            attr.columns = [sanitize_column_name(c) for c in attr.columns]
            attr["run_date"] = current_date.date()
            attr["conversion_type"] = conversion_type
            attr["conversion_window"] = lookback_window
            attribution_df_all = pd.concat([attribution_df_all, attr], ignore_index=True)

            # markov_matrix_all
            mm = attribution_markov[2].round(3).rename(
                index=lambda x: x.replace("(inicio)", "(start)"),
                columns=lambda x: x.replace("(inicio)", "(start)")
            ).reset_index()
            mm = pd.melt(mm, id_vars="index", var_name="destination", value_name="probability")
            mm.columns = ["source", "destination", "probability"]
            mm["run_date"] = current_date.date()
            mm["conversion_type"] = conversion_type
            mm["conversion_window"] = lookback_window
            markov_matrix_all = pd.concat([markov_matrix_all, mm], ignore_index=True)

            # removal_effects_all
            rem = attribution_markov[3].round(3)[["removal_effect"]]
            rem = (rem / rem.sum()) * 100
            rem.reset_index(inplace=True)
            rem.columns = ["channel", "removal_effect"]
            rem["run_date"] = current_date.date()
            rem["conversion_type"] = conversion_type
            rem["conversion_window"] = lookback_window
            removal_effects_all = pd.concat([removal_effects_all, rem], ignore_index=True)

            print("✅ Success")
        except Exception as e:
            print(f"❌ Failed: {e}")

    # ── merge user_df_all with LTV ───────────────────────────────────────────────────────
    print("\n📊 Merging with LTV...")
    ltv_df = client.query(f"SELECT * FROM {ltv_table_id}").to_dataframe()
    ltv_df = ltv_df.dropna(subset=["ltv_acquisition_capped_12m"])
    ltv_df["ltv_acquisition_capped_12m"] = ltv_df["ltv_acquisition_capped_12m"].astype(float)
    ...
    # ── NEW: compute channel-level average LTV ────────────────────────────────────────
    average_ltv_per_channel = process_user_df(user_df_all)

    removal_effects_all = removal_effects_all.merge(
        average_ltv_per_channel,
        on=["channel", "run_date", "conversion_window", "conversion_type"],
        how="left"
    )
    ...
    for destination_table, df in dataframes.items():
        ...
        if table_exists(client, destination_table):
            run_dates = pd.to_datetime(df["run_date"].dropna().unique()).astype(str)
            for run_date in run_dates:
                delete_query = f"""
                    DELETE FROM `{destination_table}`
                ...
                try:
                    ...
                    print(f"⚠️ Could not delete partition {run_date}: {e}")
        else:
            print(f"📁 {destination_table} not found, it will be created on upload.")
...
if __name__ == "__main__":
    for conv_type in ["Trial", "Subscription"]:
        for window in [30, 60, 90]:
            print(f"\n🚀 Running: {conv_type} | {window}-day window")
            try:
                run_pipeline(conv_type, window)
            except Exception as e:
                print(f"❌ Pipeline failed for {conv_type} {window}d: {e}")
